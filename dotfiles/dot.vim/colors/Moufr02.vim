" #- Vim color file -# "{{{1
" NOTE: "{{{2
" MatchParen "{[(*)]}
" highlighting ZenkakuSpace /　/
" !date
" Guicolorscheme file
" Refer for oeNmP and candycode ...

" #- reset -# "{{{1
set background=dark
if version > 580
  highlight clear
  if exists("syntax_on")
    syntax reset
  endif
endif

"#- colorscheme name -#"{{{2
let g:colors_name = expand('<sfile>:t:r')

" #- highlighting groups for various occasions -# "{{{1
highlight Normal guifg=#cfcfcf guibg=#00051f gui=NONE
                \ ctermfg=15 ctermbg=232 cterm=NONE
highlight SpecialKey guifg=#2f0fff guibg=bg gui=NONE
                \ ctermfg=15 ctermbg=bg cterm=NONE
highlight NonText guifg=#4ef9c8 guibg=bg gui=NONE
                \ ctermfg=42 ctermbg=bg cterm=NONE
highlight Directory guifg=#3f7f8f guibg=bg gui=bold
                \ ctermfg=23 ctermbg=bg cterm=bold


" Search: "{{{2
highlight IncSearch guifg=#cfcfcf guibg=#7f0f1f gui=bold
                \ ctermfg=255 ctermbg=88 cterm=bold,underline
highlight CurSearch guifg=#5f5f0f guibg=#efcf5f gui=bold,underline
                \ ctermfg=94 ctermbg=220 cterm=bold,underline
highlight Search guifg=#cfcfcf guibg=#0f2faf gui=bold
                \ ctermfg=17 ctermbg=27 cterm=bold,underline


" Message: "{{{2
highlight MoreMsg guifg=#3f5fff guibg=#00002f gui=NONE
                \ ctermfg=26 ctermbg=bg cterm=NONE
highlight ModeMsg guifg=#ff3f5f guibg=#00002f gui=NONE
                \ ctermfg=9 ctermbg=bg cterm=NONE
highlight MsgArea guifg=#cfcfcf guibg=#0f0f2f gui=NONE
                \ ctermfg=fg ctermbg=bg cterm=NONE
highlight ErrorMsg guifg=#ff0f5f guibg=bg gui=NONE
                \ ctermfg=196 ctermbg=bg cterm=NONE
highlight WarningMsg guifg=#cf1f5f guibg=bg gui=NONE
                \ ctermfg=196 ctermbg=bg cterm=NONE
highlight Question guifg=#6ab395 guibg=#00002f gui=NONE
                \ ctermfg=9 ctermbg=bg cterm=NONE
highlight stderrMsg guifg=#8f2f3f guibg=bg gui=NONE
                \ ctermfg=85 ctermbg=bg cterm=NONE
highlight stdoutMsg guifg=#3f3f3f guibg=bg gui=NONE
                \ ctermfg=85 ctermbg=bg cterm=NONE
highlight WildMenu guifg=#ffffff guibg=#1f3f8f gui=NONE
                \ ctermfg=189 ctermbg=17 cterm=NONE


" LineNr: "{{{2
highlight LineNr guifg=#8f5faf guibg=NONE gui=NONE
                \ ctermfg=141 ctermbg=bg cterm=NONE
highlight LineNrAbove guifg=#9f4fff guibg=NONE gui=NONE
                \ ctermfg=164 ctermbg=bg cterm=NONE
highlight LineNrBelow guifg=#9f9f7f guibg=NONE gui=NONE
                \ ctermfg=103 ctermbg=bg cterm=NONE


" Statusline: "{{{2
highlight StatusLine guifg=#ffffff guibg=#0f0f5f gui=NONE
                \ ctermfg=255 ctermbg=233 cterm=NONE
highlight StatusLineNC guifg=#0f0f3f guibg=#8fafff gui=NONE
                \ ctermfg=17 ctermbg=111 cterm=NONE
highlight StatusLineTerm guifg=#0f0f3f guibg=#4f8fff gui=NONE
                \ ctermfg=17 ctermbg=69 cterm=NONE
highlight StatusLineTermNC guifg=#8f5f3f guibg=#c4b2ff gui=NONE
                \ ctermfg=95 ctermbg=183 cterm=NONE
"}}}2


highlight VertSplit guifg=#4049d9 guibg=bg gui=NONE
                \ ctermfg=25 ctermbg=bg cterm=NONE
highlight WinSeparator guifg=NONE guibg=#efefff gui=bold
                \ ctermfg=250 ctermbg=16 cterm=bold
highlight FloatBorder guifg=NONE guibg=#cfcfcf gui=bold
                \ ctermfg=250 ctermbg=16 cterm=bold
highlight Title guifg=#72dc95 guibg=#0f000f gui=NONE
                \ ctermfg=72 ctermbg=0 cterm=NONE


" Visual: "{{{2
highlight Visual guifg=#afafaf guibg=#00205f gui=NONE
                \ ctermfg=15 ctermbg=17 cterm=NONE
highlight VisualNOS guifg=#0f0f1f guibg=#aaeeee gui=underline,bold
                \ ctermfg=0 ctermbg=123 cterm=bold,underline


" Fold:"{{{2
highlight Folded guifg=#0f0f0f guibg=#5f5fff gui=NONE
                \ ctermfg=16 ctermbg=63 cterm=NONE
highlight FoldColumn guifg=#2f3faf guibg=#00000f gui=bold
                \ ctermfg=25 ctermbg=232 cterm=bold


" Diff: "{{{2
highlight DiffAdd guifg=#0f0f0f guibg=#afffaf gui=NONE
                \ ctermfg=16 ctermbg=43 cterm=NONE
highlight DiffChange guifg=#0f0f0f guibg=#cfbfff gui=NONE
                \ ctermfg=16 ctermbg=147 cterm=NONE
highlight DiffDelete guifg=#0f0f0f guibg=#ffafcf gui=NONE
                \ ctermfg=16 ctermbg=219 cterm=NONE
highlight DiffText guifg=#c9cbce guibg=bg gui=NONE
                \ ctermfg=fg ctermbg=bg cterm=NONE
highlight DiffTextAdd guifg=#1f8f5f guibg=bg gui=NONE
                \ ctermfg=79 ctermbg=bg cterm=NONE
highlight Added guifg=#2faf3f guibg=bg gui=NONE
                \ ctermfg=27 ctermbg=bg cterm=NONE
highlight Changed guifg=#af9f2f guibg=bg gui=NONE
                \ ctermfg=214 ctermbg=bg cterm=NONE
highlight Removed guifg=#ef2f5f guibg=bg gui=NONE
                \ ctermfg=124 ctermbg=bg cterm=NONE
"}}}2


highlight SignColumn guifg=#a65774 guibg=#0f0f5f gui=NONE
                \ ctermfg=52 ctermbg=17 cterm=NONE
highlight Conceal guifg=NONE guibg=#c0355f gui=NONE
                \ ctermfg=fg ctermbg=1 cterm=NONE
highlight SpecialKey guifg=#3f2faf guibg=bg gui=NONE
                \ ctermfg=57 ctermbg=bg cterm=NONE


" Popup: "{{{2
highlight MessageWindow guifg=#2f2f2f guibg=#cfcfff gui=NONE
      \ ctermfg=fg ctermbg=bg cterm=NONE
highlight PopupNotification guifg=#2f2f2f guibg=#cfcfff gui=NONE
      \ ctermfg=fg ctermbg=bg cterm=NONE
highlight PopupSelected guifg=#ffffff guibg=#2f3f8f gui=NONE
      \ ctermfg=fg ctermbg=bg cterm=NONE


" Menu: "{{{2
highlight Menu guifg=#2f2f2f guibg=#3fafcf gui=None
      \ ctermfg=fg ctermbg=bg cterm=NONE


" Quickfix: "{{{2
highlight QuickFixLine guifg=#0f0f0f guibg=#cfafff gui=bold
      \ ctermfg=236 ctermbg=141 cterm=bold


" Toolbar: "{{{2
highlight ToolbarButton guifg=#5f5f8f guibg=#cfefff gui=bold
                \ ctermfg=180 ctermbg=bg cterm=NONE
highlight ToolbarLine guifg=fg guibg=#efcfff gui=NONE
                \ ctermfg=180 ctermbg=bg cterm=NONE


" Tooltip: "{{{2
highlight Tooltip guifg=#0f0f0f guibg=#efefff gui=None
                \ ctermfg=fg ctermbg=bg cterm=NONE


" Scrollbar: "{{{2
highlight Scrollbar guifg=#2f2f2f guibg=#3fafcf gui=None
                \ ctermfg=fg ctermbg=bg cterm=NONE


" Spell: "{{{2
highlight SpellBad guibg=bg guifg=#795099 gui=undercurl
                \ ctermfg=129 ctermbg=bg cterm=undercurl
highlight SpellCap guibg=bg guifg=#506399 gui=undercurl
                \ ctermfg=57 ctermbg=bg cterm=undercurl
highlight SpellRare guibg=bg guifg=#509982 gui=undercurl
                \ ctermfg=42 ctermbg=bg cterm=undercurl
highlight SpellLocal guibg=bg guifg=#998c50 gui=undercurl
                \ ctermfg=221 ctermbg=bg cterm=undercurl


" Pmenu: "{{{2
highlight Pmenu guifg=#0f0f0f guibg=#cfc0cf gui=NONE
                \ ctermfg=232 ctermbg=182 cterm=NONE
highlight PmenuSel guifg=#cfc0cf guibg=#0f0f2f gui=NONE
                \ ctermfg=15 ctermbg=238 cterm=NONE
highlight Pmenukind guifg=#0f0f0f guibg=#cfcfff gui=NONE
                \ ctermfg=16 ctermbg=183 cterm=NONE
highlight PmenukindSel guifg=#ffffff guibg=#003f8f gui=NONE
                \ ctermfg=15 ctermbg=18 cterm=NONE
highlight PmenuExtra guifg=#0f0f0f guibg=#cfcfff gui=NONE
                \ ctermfg=16 ctermbg=183 cterm=NONE
highlight PmenuExtraSel guifg=#ffffff guibg=#003f8f gui=NONE
                \ ctermfg=15 ctermbg=18 cterm=NONE
highlight PmenuMatch guifg=#0f0f0f guibg=#cfefff gui=NONE
                \ ctermfg=fg ctermbg=16 cterm=NONE
highlight PmenuMatchLead guifg=#0f0f0f guibg=#cfffff gui=NONE
                \ ctermfg=16 ctermbg=81 cterm=NONE
highlight PmenuMatchSel guifg=#ffffff guibg=#003faf gui=NONE
                \ ctermfg=15 ctermbg=17 cterm=NONE
highlight ComplMatchIns guifg=#3f3f3f guibg=#cfcfff gui=NONE
                \ ctermfg=fg ctermbg=189 cterm=NONE
highlight PmenuSbar guifg=#9c9c9c guibg=#0f0f2f gui=NONE
                \ ctermfg=250 ctermbg=235 cterm=NONE
highlight PmenuThumb guifg=#0f0f0f guibg=#1126f6 gui=NONE
                \ ctermfg=fg ctermbg=21 cterm=NONE


" Tabline: "{{{2
highlight TabLine guifg=#5fafff guibg=#00050f gui=underline
                \ ctermfg=75 ctermbg=232 cterm=underline
highlight TabLineFill guifg=#0f1fff guibg=#00050f gui=bold,underline
                \ ctermfg=19 ctermbg=232 cterm=underline
highlight TabLineSel  guifg=#cfcfcf guibg=#0f1f3f gui=NONE
                \ ctermfg=15 ctermbg=17 cterm=NONE


" Tabpanel: "{{{2
highlight TabPanel guifg=#0f1fff guibg=#00050f gui=NONE
                \ ctermfg=75 ctermbg=232 cterm=NONE
highlight TabPanelFill guifg=#0f1fff guibg=#00050f gui=NONE
                \ ctermfg=19 ctermbg=232 cterm=NONE
highlight TabPanelSel  guifg=#cfcfcf guibg=#0f1f3f gui=NONE
                \ ctermfg=15 ctermbg=17 cterm=NONE
highlight TabPanelTitle guifg=#ffffff guibg=#0f3f8f gui=NONE
                \ ctermfg=232 ctermbg=244 cterm=NONE
highlight TabPanelCurMark guifg=#ffffff guibg=#0f3f8f gui=NONE
                \ ctermfg=232 ctermbg=244 cterm=NONE


" Cursor "{{{2
highlight ColorColumn guifg=NONE guibg=#00001f gui=NONE
                \ ctermfg=NONE ctermbg=17 cterm=NONE
highlight CursorColumn guifg=NONE guibg=#00000f gui=NONE
                \ ctermfg=NONE ctermbg=17 cterm=NONE
highlight lCursor guifg=#0f0f0f guibg=#cfa3ff gui=NONE
                \ ctermfg=fg ctermbg=139 cterm=NONE
highlight Cursor guifg=#0f0f0f guibg=#cfbfff gui=NONE
                \ ctermfg=16 ctermbg=147 cterm=NONE
highlight CmdlineCursor guifg=#000000 guibg=#cfffff gui=NONE
                \ ctermfg=NONE ctermbg=250 cterm=NONE


" IME status color for cursor "{{{3
if has('multi_byte_ime') || has('xim')
  highlight CursorIM guifg=#0f0f0f guibg=#8f1f3f gui=NONE
                \ ctermfg=fg ctermbg=124 cterm=NONE
endif


" CUrsorline: "{{{2
highlight CursorLine guifg=NONE guibg=#000f5f gui=NONE
                \ ctermfg=NONE ctermbg=17 cterm=NONE
highlight CursorLineNr guifg=#3f5faf guibg=#8fafcf gui=NONE
                \ ctermfg=25 ctermbg=111 cterm=bold
highlight CursorLineFold guifg=NONE guibg=#cfcfff gui=bold
                \ ctermfg=15 ctermbg=189 cterm=bold
highlight CursorLineSign guifg=NONE guibg=#8faf8f gui=bold
                \ ctermfg=249 ctermbg=108 cterm=bold


" Terminal: "{{{2
if has('terminal')
  highlight Terminal guifg=fg guibg=bg gui=NONE
                  \ ctermfg=fg ctermbg=bg cterm=NONE
  " Terminal color
  let g:terminal_ansi_colors = [
    \ "#0c0c0c", "#850f1f", "#13710e", "#819c00",
    \ "#00378a", "#881798", "#3a96dd", "#8f8f8f",
    \ "#767676", "#872836", "#16960c", "#6a7f84",
    \ "#3b789f", "#94005e", "#61d6d6", "#989898"
    \ ]
endif



" toolbar "{{{2
highlight ToolbarButton guifg=#3f1f8f guibg=#afafff gui=NONE
          \ ctermfg=17 ctermbg=147 cterm=NONE
highlight ToolbarLine guifg=#5f0f0f guibg=#df3fcf gui=NONE
          \ ctermfg=52 ctermbg=127 cterm=NONE




" #- syntax highlighting group -# "{{{1
highlight MatchParen guifg=#000000 guibg=#2f3faf gui=bold,underline
                \ ctermfg=232 ctermbg=33 cterm=bold,underline
highlight comment guifg=#2f8fff guibg=bg gui=NONE
                \ ctermfg=27 ctermbg=bg cterm=NONE
highlight Constant guifg=#af1f8f guibg=bg gui=NONE
                \ ctermfg=127 ctermbg=bg cterm=NONE
highlight Special guifg=#ffdfcf guibg=bg gui=NONE
                \ ctermfg=215 ctermbg=bg cterm=NONE
highlight Identifier guifg=#2faf7f guibg=bg gui=NONE
                \ ctermfg=36 ctermbg=bg cterm=NONE
highlight Statement guifg=#0f6fff guibg=bg gui=NONE
                \ ctermfg=26 ctermbg=bg cterm=NONE
highlight PreProc guifg=#af3faf guibg=bg gui=NONE
                \ ctermfg=163 ctermbg=bg cterm=NONE
highlight type guifg=#2fc98f guibg=bg gui=NONE
                \ ctermfg=42 ctermbg=bg cterm=NONE
highlight Ignore guifg=#0f1faf guibg=bg gui=NONE
                \ ctermfg=21 ctermbg=bg cterm=bold
highlight Error guifg=#af1f5f guibg=bg gui=NONE
                \ ctermfg=9 ctermbg=bg cterm=NONE
highlight string guifg=#cf3f8f guibg=bg gui=NONE
                \ ctermfg=161 ctermbg=bg cterm=NONE
highlight Todo guifg=#ff8f3f guibg=#0f0f2f gui=bold,underline
                \ ctermfg=214 ctermbg=233 cterm=bold,underline
highlight underlined guifg=#2f8fff guibg=bg gui=bold,underline
                \ ctermfg=12 ctermbg=bg cterm=bold,underline

" #- END -# "{{{1
" vim: fen:fdm=marker
" vim: filetype=vim


